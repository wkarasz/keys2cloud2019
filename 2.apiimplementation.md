...
4)	Select **+ Import app**.
5)	Navigate to where you extracted the connected customer experience artifacts and choose or drag **CustomerExperienceAirport/ApiSpec/v1/itineraries.json** into upload files and select Upload. 
	1) Or redownload the [itineraries.json](project/ApiSpec/v1/itineraries.json)
...
1)	Select **V1Itineraries_reservationid_GET**.  GET /itineraries/{reservationid} implementation will be opened.  Here you see the orchestration that invokes reservations, passengers, flights, status APIs.  You are now going invoke an API call to aircrafts in the next steps.
